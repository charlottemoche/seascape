...
export default function TipCard() {
  ...
  const messageText =
    colorScheme === "dark" ? Colors.custom.blue : Colors.custom.green;
    ...
    <View style={styles.container}>
      ...
      {!hasTipped && (
        <Text style={styles.text}>
          This app was made by one person, slowly and with care. No
          subscriptions, no ads, just a tiny project from me to you. Tips help
          me keep it alive!
        </Text>
      )}

      {hasTipped ? (
        <Text style={[styles.text, { color: messageText }]}>{message}</Text>
      ) : (
      ...
      <Text style={[styles.text, { marginTop: 20 }]}>
        Please note that if you are not logged in, your tip status and new fish
        colors will only be accessible on this device.
      </Text>
    </View>
  );
}
