...
export default function ProfileScreen() {
  ...
  const starfishImage = colorScheme === "dark" ? whiteStarfish : starfish;

  const bubblesImage = colorScheme === "dark" ? whiteBubbles : bubbles;
  ...
  const linkColor =
    colorScheme === "dark" ? Colors.custom.blue : Colors.custom.darkBlue;
  const greyBorder =
    colorScheme === "dark" ? Colors.custom.darkGrey : Colors.custom.grey;
  const selectedTab =
    colorScheme === "dark" ? Colors.custom.darkGrey : Colors.custom.white;
  const unselectedTab =
    colorScheme === "dark" ? Colors.dark.background : Colors.custom.transparent;
  ...
  const highScore = Math.max(
    ...
    hasLoadedLocalHS ? localHighScore : 0
  );

  const profileStillLoading =
    userLoading || !sessionChecked || (user && !profile);
    ...
    return (
      <View style={styles.actions}>
        <View style={styles.actionWrapper}>
          ...
          <Text>Starfish hug</Text>
        </View>
        <View style={styles.actionWrapper}>
          <Text>
            <Image source={bubblesImage} style={[styles.icon, { width: 18 }]} />
          </Text>
          <Text>Breathe reminder</Text>
        </View>
      </View>
    );
  }
  ...
  const deleteAccount = async () => {
    ...
    const res = await fetch(
      "https://kycsdqkgoqroyxdytgil.supabase.co/functions/v1/delete-user",
      {
        method: "DELETE",
    ...
    >
      ...
      >
        ...
        <View style={styles.container}>
          <View style={{ display: tab === "profile" ? "flex" : "none" }}>
            {isLoggedIn ? (
              <View
                style={[styles.profileSection, { backgroundColor: cardColor }]}
              >
                <View style={styles.textRow}>
                  <Text
                    style={[
                      styles.label,
                      { borderBottomColor: greyBorder, color: textColor },
                    ]}
                  >
                    Email
                  </Text>
              ...
              {isLoggedIn ? (
                <>
                  <View
                    style={[
                      styles.tabBar,
                      styles.tabBarFriends,
                      { borderColor: greyBorder, marginBottom: 30 },
                    ]}
                  >
                    {(["list", "add", "requests"] as const).map(
                      (key, idx, arr) => (
                        <Pressable
                          key={key}
                          onPress={() => {
                          ...
                          <View style={styles.tabs}>
                            <Text
                              style={[
                                ...
                                {
                                  color:
                                    friendSubTab === key ? "#000" : textColor,
                                },
                              ]}
                            >
                              {key === "add"
                  ...
                  <View
                    style={{
                      display: friendSubTab === "list" ? "flex" : "none",
                    }}
                  >
                    <ActionLegend />
                  </View>
      ...
      <View style={{ backgroundColor: cardColor, maxHeight: 50 }}>
        <View
          style={[
            styles.footer,
            { backgroundColor: cardColor, borderColor: greyBorder },
          ]}
        >
