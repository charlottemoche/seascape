...
const mockUpdateStreak = () =>
  jest.spyOn(streakModule, "updateStreak").mockImplementation(async () => {
    ...
    const diffDays = Math.floor(
      (now.getTime() - today.getTime()) / (1000 * 60 * 60 * 24)
    );
...
const getJournalScreen = async () => {
  ...
  const openModalPressable = await waitFor(() =>
    utils.getByText("Write your thoughts here...")
  );
  fireEvent.press(openModalPressable);

  const modalInput = await waitFor(() =>
    utils.getByTestId("journal-modal-input")
  );
...
describe("journal", () => {
  ...
  it("does not lose the streak the next morning", async () => {
    ...
    const { journalStreak } = await streakModule.fetchStreaks(
      "test-user",
      "America/New_York"
    );
